...
if [ -n "$WEBHOOK_URL" ]; then
    ...
    curl -X POST -H 'Content-type: application/json' --data '{"blocks":[{"type":"section","text":{"type":"mrkdwn","text":"'"$JOB_STATUS_EMOJI"' '"$POD_NAME"' '"$JOB_STATUS_TEXT"'"}},{"type":"section","fields":[{"type":"mrkdwn","text":"*Experiment Batch:*\n'"$EXP_BATCH"'"},{"type":"mrkdwn","text":"*Experiment Name:*\n'"$EXP_NAME"'"},{"type":"mrkdwn","text":"*Started:*\n'"$START_TIME"'"},{"type":"mrkdwn","text":"*Completed:*\n'"$END_TIME"'"},{"type":"mrkdwn","text":"*Actor:*\n'"$EXP_ACTOR"'"}]}]}' "$WEBHOOK_URL"
fi
